...
reviewer が blocker を出して 3 ラウンドまで自動修正します。それでも残れば `projects/<name>/review_report.md` を読んで、手で `projects/<name>/input.json` の `bg_query` を編集してから `/shortvideo-loop <name>` を再実行してください。

### 台本がいまいち

このツールの planner はざっくり叩き台しか出しません。本格的な台本にしたい場合は:

1. `/shortvideo-planner <name>` で叩き台を作る
2. `projects/<name>/input.json` を手で編集 (キャッチコピー・ナレーション本文・bg_query の細かい指定)
3. `/shortvideo-loop <name>` で動画化

「アカウント設計 / 競合分析 / 伸びている投稿の構成パターン」等は AI に渡せば planner に反映できます。お題と一緒に貼り付けてください。
